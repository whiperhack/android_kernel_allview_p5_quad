...
//Gionee yanggy 2012-03-13 modify for ICS start
static struct i2c_board_info __initdata i2c_lis3dh={ I2C_BOARD_INFO(LIS3DH_DEV_NAME, (LIS3DH_I2C_SLAVE_ADDR>>1))};
...
//Gionee yanggy 2012-03-13 modify for ICS end
...
static struct i2c_driver lis3dh_i2c_driver = {
    .driver = {
 //       .owner          = THIS_MODULE,   //delete by wanght at 2012-08-07 for ICS
...
//	.address_data = &lis3dh_addr_data,   //delete by wanght at 2012-08-7 for ICS
};
...
// LuckAs add
    {{ 15, 6}, 64},   /*+/-2g  in 8-bit resolution:  15.6 mg/LSB*/
    {{ 31, 2}, 32},   /*+/-4g  in 8-bit resolution:  31.2 mg/LSB*/
    {{62, 5},  16},   /*+/-8g  in 8-bit resolution: 62.5 mg/LSB*/
// LuckAs end
...
// LuckAs disable errors compile
//Gionee wanght 2012-03-16 open it begin
//#if defined(GN_MTK_BSP)
static int hwmsen_read_byte_st(struct i2c_client *client, u8 reg_addr, u8 *data)
{
...
static int hwmsen_write_byte_st(struct i2c_client *client, u8 *data,int len)
{
...
//Gionee wanght 2012-03-16 opent it end
// LuckAs end
...
static void dumpReg(struct i2c_client *client)
{
  ...
  {
    //dump all
    hwmsen_read_byte_st(client,addr,&regdata);
...
static int LIS3DH_SetDataResolution(struct lis3dh_i2c_data *obj)
{
	int err;
	u8  dat, reso;
	if(err = hwmsen_read_byte_st(obj->client, LIS3DH_REG_CTL_REG4, &dat))
	{
...
static int LIS3DH_ReadData(struct i2c_client *client, s16 data[LIS3DH_AXES_NUM])
{
	...
	{
		if(hwmsen_read_byte_st(client, LIS3DH_REG_OUT_X, &buf[0] ))
	    {
		   GSE_ERR("read  G sensor data register err!\n");
		     return -1;
	    }
		if(hwmsen_read_byte_st(client, LIS3DH_REG_OUT_X+1, &buf[1]))
	    {
		   GSE_ERR("read  G sensor data register err!\n");
		     return -1;
	    }
	...
	if(hwmsen_read_byte_st(client, LIS3DH_REG_OUT_Y, &buf[2]))
	    {
		   GSE_ERR("read  G sensor data register err!\n");
		     return -1;
	    }
	if(hwmsen_read_byte_st(client, LIS3DH_REG_OUT_Y+1, &buf[3]))
	    {
		   GSE_ERR("read  G sensor data register err!\n");
		     return -1;
	    }
	...
	if(hwmsen_read_byte_st(client, LIS3DH_REG_OUT_Z, &buf[4]))
	    {
		   GSE_ERR("read  G sensor data register err!\n");
		     return -1;
	    }

	if(hwmsen_read_byte_st(client, LIS3DH_REG_OUT_Z+1, &buf[5]))
	    {
		   GSE_ERR("read  G sensor data register err!\n");
		     return -1;
	    }
...
/*
	...
	res = hwmsen_write_byte_st(client, databuf, 0x1);
	if(res <= 0)
	{
...
static int LIS3DH_SetPowerMode(struct i2c_client *client, bool enable)
{
	...
	if(hwmsen_read_byte_st(client, addr, &databuf[0]))
	{
	...
	res = hwmsen_write_byte_st(client, databuf, 0x2);
...
*/
 ...
 //   res = hwmsen_write_byte(client,LIS3DH_REG_CTL_REG1,0x07);
    ...
    msleep(20);   //gionee added by wangsx 20120303
...
static ssize_t show_power_status(struct device_driver *ddri, char *buf)
{
	...
	obj = i2c_get_clientdata(client);
	hwmsen_read_byte_st(client,LIS3DH_REG_CTL_REG1,&data);
	data &= 0x08;
	data = data>>3;
	...
	switch (command)
	{
		case SENSOR_DELAY:
			...
			else
			{
				...
				{
					//sample_delay = ~LIS3DH_BW_100HZ;   //modified by wangsx 20120303
                    sample_delay = LIS3DH_BW_100HZ; 
				}
				else
				{
					//sample_delay = ~LIS3DH_BW_50HZ;     //modified by wangsx 20120303
                    sample_delay = LIS3DH_BW_50HZ;
				}
				
				err = LIS3DH_SetBWRate(priv->client, LIS3DH_BW_100HZ);
...
/*----------------------------------------------------------------------------*/
//modified by yanggy 2012-03-13 for ICS
...
static int lis3dh_suspend(struct i2c_client *client, pm_message_t msg) 
{
	...
	{   
		...
		//read old data
		if ((err = hwmsen_read_byte_st(client, LIS3DH_REG_CTL_REG1, &dat))) 
		{
           GSE_ERR("write data format fail!!\n");
           return err;
        }
		...
		if(err==hwmsen_write_byte_st(client, databuf, 0x2))
		{
...
static void lis3dh_early_suspend(struct early_suspend *h) 
{
	...
	atomic_set(&obj->suspend, 1); 
	/*
	if(err = hwmsen_write_byte_sr(obj->client, LIS3DH_REG_POWER_CTL, 0x00))
	{
		GSE_ERR("write power control fail!!\n");
		return;
	}  
...
static int  lis3dh_local_init(void)
{
	...
	GSE_ERR("success to lis3dh_probe driver");
	LIS3DH_power(hw, 1);
...
static void __exit lis3dh_exit(void)
{
	GSE_FUN();
// LuckAs disable
//    hwmsen_gsensor_del(&lis3dh_init_info);       //Gionee yanggy 2012-04-01 add for MTK_sensor_compability policy 
// LuckAs end
}
/*----------------------------------------------------------------------------*/
