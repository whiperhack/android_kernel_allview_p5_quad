...
int AP3216C_write_ps_thd2(struct i2c_client *client, u16 afocal)
{
	struct AP3216C_priv *obj = i2c_get_clientdata(client);
        u8 databuf[2];    
	int res = 0;
	...
	printk("[chj]AP3216C_write_ps_thd2 afocal=%d ",afocal);
	//databuf[0] = 0x28;    
	//databuf[1] = afocal & 0x03;
	//res = i2c_master_send(client, databuf, 0x2);
	//if(res <= 0)
	//{
	//	goto EXIT_ERR;
...
static int AP3216C_init_client(struct i2c_client *client)
{
	...
	if(res <= 0)
	{
        ...
        AP3216C_write_ps_thd(client,atomic_read(&obj->ps_thd_val_low),atomic_read(&obj->ps_thd_val_high));
/*
	if(ps_cali.valid==1)
        {
        ...
        {
             thd_low =atomic_read(&obj->ps_thd_val_low);
             thd_high=atomic_read(&obj->ps_thd_val_high);
        }
...
static int AP3216C_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd)
	{
		...
		case ALSPS_GET_PS_THD:    //ps_threshold
			...
			AP3216C_write_ps_thd(obj->client, atomic_read(&obj->ps_thd_val_low),atomic_read(&obj->ps_thd_val_high));
			break;
		 case ALSPS_SET_PS_CALI2:
			dat = (void __user*)arg;
			if(dat == NULL)
			{
				APS_LOG("dat == NULL\n");
			...
			if(copy_from_user(&afocal,dat, sizeof(afocal)))
			{
				APS_LOG("copy_from_user\n");
				err = -EFAULT;
				break;	  
			}
			printk("[LONG][ALS/PS] ALSPS_SET_PS_CALI2 %d\n\r",afocal);
			AP3216C_write_ps_thd2(obj->client, afocal);
			break;
/************************maliyan add 2012-11-12 end*************************************/
