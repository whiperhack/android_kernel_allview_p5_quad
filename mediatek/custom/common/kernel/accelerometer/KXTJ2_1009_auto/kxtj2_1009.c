	...
	switch (command)
	{
		...
		case SENSOR_ENABLE:
			...
			else
			{
				...
				{
					err = KXTJ2_1009_SetPowerMode( priv->client, !((bool)atomic_read(&priv->sensor_power)));
				}
...
static int kxtj2_1009_resume(struct i2c_client *client)
{
	...
	if (0)//(err = kxtj2_1009_init_client(client, 0))  //longxuewei
	{
...
static void kxtj2_1009_late_resume(struct early_suspend *h)
{
	...
	KXTJ2_1009_power(obj->hw, 1);
	if (0)//(err = kxtj2_1009_init_client(obj->client, 0))  //longxuewei
	{
		GSE_ERR("initialize client fail!!\n");
		return;        
	}
...
static int kxtj2_1009_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
...
#endif 
	kxtj2_1009_init_flag = 0;/* Add for auto detect feature */
	GSE_LOG("%s: OK\n", __func__);    
	return 0;
	...
	kfree(obj);
	exit:
	kxtj2_1009_init_flag = -1;/* Add for auto detect feature */
	GSE_ERR("%s: err = %d\n", __func__, err);        
	return err;
}
